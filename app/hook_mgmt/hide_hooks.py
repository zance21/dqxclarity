...
from signatures import loading_pointer, loading_offsets, cutscene_pattern
...
def load_unload_hooks(hook_list: list, debug: bool):
    ...
    while True:
        try:
            state_byte = read_bytes(
                get_ptr_address(base_address + loading_pointer, loading_offsets), 1
            )
            ...
            if (
                cutscene_byte != b"\x00"
            ):  # separate check as state byte can't see we're in a cutscene
                for i in range(
                    300
                ):  # check for 1~ seconds to finish loading and account for user skipping
                    time.sleep(0.01)
                    cutscene_byte = read_bytes(cutscene_addr, 1)
                    if i == 299:
                        for hook in hook_list:
