...
from memory import dqx_mem, read_bytes, write_bytes, write_string, pattern_scan
...
def write_pre_hook_registers() -> dict:
    ...
    write_bytes(
        mov_insts, b"\xA3" + pack_to_int(reg_values) + b"\x90"
    )  # mov [reg_values], eax then nop
    write_bytes(
        mov_insts + 6, b"\x89\x1D" + pack_to_int(reg_values + 4)
    )  # mov [reg_values+6], ebx
    write_bytes(
        mov_insts + 12, b"\x89\x0D" + pack_to_int(reg_values + 8)
    )  # mov [reg_values+12], ecx
    write_bytes(
        mov_insts + 18, b"\x89\x15" + pack_to_int(reg_values + 12)
    )  # mov [reg_values+18], edx
    write_bytes(
        mov_insts + 24, b"\x89\x35" + pack_to_int(reg_values + 16)
    )  # mov [reg_values+24], esi
    write_bytes(
        mov_insts + 30, b"\x89\x3D" + pack_to_int(reg_values + 20)
    )  # mov [reg_values+30], edi
    write_bytes(
        mov_insts + 36, b"\x89\x2D" + pack_to_int(reg_values + 24)
    )  # mov [reg_values+36], ebp
    write_bytes(
        mov_insts + 42, b"\x89\x25" + pack_to_int(reg_values + 28)
    )  # mov [reg_values+42], esp

    addresses_dict = dict()
    addresses_dict[
        "begin_mov_insts"
    ] = mov_insts  # address where register backups occur
    addresses_dict["begin_hook_insts"] = (
        mov_insts + 48
    )  # address where to start hook instructions
    addresses_dict[
        "begin_reg_values"
    ] = reg_values  # address where to restore register backups
...
def write_post_hook_registers(
    pre_register_value_addr: int, hook_instr_end: int
) -> dict:
    """
    ...
    """
    write_bytes(
        hook_instr_end, b"\xA1" + pack_to_int(pre_register_value_addr) + b"\x90"
    )  # mov eax, [pre_register_value_addr] then nop
    write_bytes(
        hook_instr_end + 6, b"\x8B\x1D" + pack_to_int(pre_register_value_addr + 4)
    )  # mov ebx, [pre_register_value_addr+6]
    write_bytes(
        hook_instr_end + 12, b"\x8B\x0D" + pack_to_int(pre_register_value_addr + 8)
    )  # mov ecx, [pre_register_value_addr+12]
    write_bytes(
        hook_instr_end + 18, b"\x8B\x15" + pack_to_int(pre_register_value_addr + 12)
    )  # mov edx, [pre_register_value_addr+18]
    write_bytes(
        hook_instr_end + 24, b"\x8B\x35" + pack_to_int(pre_register_value_addr + 16)
    )  # mov esi, [pre_register_value_addr+24]
    write_bytes(
        hook_instr_end + 30, b"\x8B\x3D" + pack_to_int(pre_register_value_addr + 20)
    )  # mov edi, [pre_register_value_addr+30]
    write_bytes(
        hook_instr_end + 36, b"\x8B\x2D" + pack_to_int(pre_register_value_addr + 24)
    )  # mov ebp, [pre_register_value_addr+36]
    write_bytes(
        hook_instr_end + 42, b"\x8B\x25" + pack_to_int(pre_register_value_addr + 28)
    )  # mov esp, [pre_register_value_addr+42]

    addresses_dict = dict()
    addresses_dict["end_mov_insts"] = (
        hook_instr_end + 48
    )  # address where register restore ends
...
def generic_detour(
    ...
    if shellcode:
        pyrun_simplestring_addr = pattern_scan(
            pyrun_simplestring, module="python39.dll"
        )
        ...
        bytecode = b"\xE8" + calc_rel_addr(
            pre_hook["begin_hook_insts"], py_initialize_ex_addr
        )  # call py_initialize_ex_addr
        bytecode += b"\x68" + bytes(pack_to_int(shellcode_addr))  # push shellcode_addr
        bytecode += b"\xE8" + calc_rel_addr(
            pre_hook["begin_hook_insts"] + len(bytecode), pyrun_simplestring_addr
        )  # push py_run_simple_string_addr
    ...
    post_hook = write_post_hook_registers(
        pre_hook["begin_reg_values"], pre_hook["begin_hook_insts"] + len(bytecode)
    )
    ...
    bytecode = b"\xE9" + calc_rel_addr(
        post_hook["end_mov_insts"] + num_bytes_to_steal, detour_address + count
    )
    ...
    return convert_dict(
        ...
        stolen_bytecode,
        hook_bytecode,
        pre_hook["begin_mov_insts"],
        post_hook["end_mov_insts"],
    )
    ...
    for hook in hooks:
        if hook["hook_name"] in unhookers:
            ...
            unhook_bytecode_with_jump = (
                ...
                + b"\xE9"
                + calc_rel_addr(
                    hook["hook_begin_addr"] + len(unhook_bytecode),
                    hook["detour_address"] - stolen_byte_len + count,
                )
            )
