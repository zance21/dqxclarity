...
def get_latest_from_weblate():
    ...
    try:
        ...
        messageBoxFatalError(
            "Timeout",
            'Timed out trying to reach github.com. Relaunch Clarity without "Pull latest files from weblate" and try again.',
        )
...
def translate():
    ...
    with alive_bar(
        list_length, title="Translating..", theme="musical", length=20
    ) as bar:
...
def write_adhoc_entry(start_addr: int, hex_str: str) -> dict:
    ...
    if csv_result:
        ...
        begin_address = get_start_of_game_text(
            start_addr
        ...
        write_file(
            "new_adhoc_dumps", "new_hex_dict.csv", "a", f"{filename},{hex_result}\n"
        )
...
def scan_for_npc_names():
    ...
    while True:
        try:
            index_list = pattern_scan(
                pattern=npc_monster_byte_pattern, return_multiple=True
            )
...
def scan_for_player_names():
    ...
    while True:
        try:
            player_list = pattern_scan(
                pattern=player_name_byte_pattern, return_multiple=True
            )
            if player_list == []:
                continue
            ...
            for address in player_list:
                ...
                write_bytes(
                    player_name_address, b"\x04" + romaji_name.encode("utf-8") + b"\x00"
                )
        ...
        try:
            ...
            for index_address in index_list:
                if read_bytes(index_address - 2, 1) != b"\x69":
                    hex_result = split_hex_into_spaces(
                        str(read_bytes(index_address, 64).hex())
                    )
                    csv_result = query_csv(hex_result)
                    if csv_result:
                        file = csv_result["file"]
                        if "adhoc_wd_" in file:
                            ...
                            if text_address:
                                try:
                                    # this just tests that we can decode what we should be writing
                                    foot_address = find_first_match(
                                        text_address, foot_pattern
                                    )
                                    game_hex = read_bytes(
                                        text_address, foot_address - text_address
                                    )
                                ...
                                if (
                                    ...
                                    write_bytes(text_address, hex_to_write)
                                    write_bytes(
                                        index_address - 2, b"\x69"
                                    )  # our mark that we wrote here so we don't write again. nice.
                        ...
                        if text != prev_text:
                            ...
                            if detect_lang(text):
                                ...
                                if result:
                                    ...
                                    write_bytes(
                                        address + 16, translated_text.encode() + b"\x00"
                                    )
                        else:
                            time.sleep(5)
            else:
...
def dump_game_file(start_addr: int, num_bytes_to_read: int):
    ...
    json_data_ja = json.dumps(
        jsondata_ja, indent=2, sort_keys=False, ensure_ascii=False
    )
    json_data_en = json.dumps(
        jsondata_en, indent=2, sort_keys=False, ensure_ascii=False
    )
...
def check_for_updates():
    ...
    try:
        ...
        logger.warning(
            f"Failed to check latest version. Running anyways.\nMessage: {e}"
        )
        return

    if github_request.text != exe_sha:
        logger.info(
            "An update is available at https://github.com/jmctune/dqxclarity/releases",
            fg="green",
        )
    else:
