...
def write_dict(file: str, new_name: str, skip_file_read=False, hex_bytes=""):
    ...
    if not skip_file_read:
        with open(f"dqx_out/{file}", "rb") as f:
            ...
            the_bytes = f.read(
                64
            )  # get unique bytes for hex dict. 64 is arbitrary, but clarity expects it
...
def compare_jsons(source: str):
    ...
    orig_list = []
    count = 0
    # grab first 5 entries from file we already have
    for item in orig:
        count += 1
        ...
        if count == 5:
            break
    # match the previous 5 entries against every json file until we find a match
    for file in listdir("../../json/_lang/en"):
        ...
        for item in new:
            ...
            if list_len == count:
                ...
                key == orig_list[count] and list_len > count
            ):  # some files don't have 5 entries
                count += 1
                continue
            else:
                break
...
def dump_all_game_files():
    ...
    with alive_bar(
        len(game_file_addresses),
        title="Dumping from memory..",
        theme="smooth",
        length=20,
    ) as bar:
        for address in game_file_addresses:
            ...
            start_addr = get_start_of_game_text(
                address
            )  # make sure we start on the first byte of the first letter
            if start_addr is not None:
                ...
                if end_addr is not None:
                    ...
                    if bytes_to_read < 0:
                        continue
                    game_data = (
                        read_bytes(start_addr, bytes_to_read).rstrip(b"\x00").hex()
                    )
                    ...
                    json_data_ja = json.dumps(
                        jsondata_ja, indent=4, sort_keys=False, ensure_ascii=False
                    )

                    json_data_en = json.dumps(
                        jsondata_en, indent=4, sort_keys=False, ensure_ascii=False
                    )
                    ...
                    if matching_file:
                        copy2(
                            f"json_out/en/{file}",
                            f"hyde_json_merge/dst/{matching_file}",
                        )  # stage new file for port
                        copy2(
                            f"json_out/ja/{file}", f"json/_lang/ja/{matching_file}"
                        )  # move new ja to new folder
                        copy2(
                            f"../../json/_lang/en/{matching_file}",
                            "hyde_json_merge/src",
                        )  # stage orig file for port
                        write_dict(
                        ...
                        copy2(
                            f"json_out/en/{file}", f"json/_lang/en"
                        )  # move new en to new folder
                        copy2(
                            f"json_out/ja/{file}", f"json/_lang/ja"
                        )  # move new ja to new folder
                        write_dict(
                            f"json\_lang\en\{file}",
                            f"json\_lang\en\{file}",
...
if __name__ == "__main__":
    ...
    with alive_bar(
        dir_count, title="Analyzing EVTs..", theme="smooth", length=20
    ) as bar:
        for file in listdir("dqx_out"):
            bar()
            if query_csv(
                f"dqx_out/{file}", compare_type="filename"
            ):  # if we dumped it from memory already, don't do double work
                continue
            ...
            if matching_file:
                ...
                if check_csv:
                    continue
                copy2(
                    f"json_out/en/{json_file}", f"hyde_json_merge/dst/{matching_file}"
                )  # stage new file for port
                copy2(
                    f"json_out/ja/{json_file}", f"json/_lang/ja/{matching_file}"
                )  # move new ja to new folder
                copy2(
                    f"../../json/_lang/en/{matching_file}", "hyde_json_merge/src"
                )  # stage orig file for port
                write_dict(f"{file}", f"json\_lang\en\{matching_file}")
            else:
                # nothing to port b/c new file. move new files and do nothing
                copy2(
                    f"json_out/en/{json_file}", f"json/_lang/en"
                )  # move new en to new folder
                copy2(
                    f"json_out/ja/{json_file}", f"json/_lang/ja"
                )  # move new ja to new folder
