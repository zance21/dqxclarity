...
def sanitized_dialog_translate(translation_service, is_pro, dialog_text, api_key, region_code) -> str:
    ...
    for item in output:
        ...
        if item == '<br>':  # we'll manage our own line breaks later
            final_string += ' '
            continue
        alignment = ['<center>', '<right>', '<left>']  # center and right aligned text doesn't work well in this game with ascii. left is useless
        ...
        if re.search('(<(?!%).+?>)', item):  # don't capture variable tags. ex: <%nC_GOLD>
            ...
            if any(x in item for x in puncs):
                ...
                sanitized = re.sub('\|', ' ', sanitized)  # we need these, but they mess up the translation. put them back later
